...
import hello.form.AppUserForm;
import hello.util.EncryptedPasswordUtils;
...
public class AppUserDAO {
	...
	public AppUser findByUserId(Long userId) {
		try {
			String sql = "SELECT au FROM " + AppUser.class.getName() + " au" + "WHERE au.USER_ID = :userId";
	...
	public AppUser findByEmail(String email) {
		try {
			String sql = "SELECT au FROM " + AppUser.class.getName() + " au" + "WHERE au.EMAIL = :email";
			Query query = entityManager.createQuery(sql, AppUser.class);
	...
	private String findAvailableUserName(String userName_prefix) {
		AppUser account = this.findByUserName(userName_prefix);
		if (account == null) {
