...
import hello.routing.MyRoutingDataSource;

/*
 * Theo mặc định Spring Boot sẽ tự động cấu hình một DataSource, 
 * vì vậy bạn cần phải vô hiệu hóa nó để có thể tự cấu hình 
 * các DataSource của bạn. Các cấu hình tự động này bao gồm:
...
// Load to Environment (see resources/datasource-cfg.properties).
@PropertySources({ @PropertySource("classpath:datasource-cfg.properties") })
public class O7planBootMultiDataSourceApplication {
	// Lưu trữ các giá thuộc tính load bởi @PropertySource.
	@Autowired
	private Environment env;

	public static void main(String[] args) {
		SpringApplication.run(O7planBootMultiDataSourceApplication.class, args);
	}
